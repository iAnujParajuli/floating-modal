...
var drop = function(info){var o = {
   ...
   open: false,
   html: {
      select: jQuery(info.selector)[0],
      options: jQuery(info.selector + ' option'),
      parent: undefined,
   },
   init: function(){
      //Setup Drop HTML
      this.html.parent = jQuery(info.selector)[0].parentNode;
   ...
   addOption: function(e, element){ 
      ...
      var dataIndex = e.originalTarget.dataset.index;
      jQuery(info.selector + " option")[dataIndex].selected = true;
      this.render();
   },
   ...
   renderDrop: function(){ 
      ...
      this.selected.forEach(function(select, index){ 
         ...
         var childCloseHTML = $.template(
            '<i class="dashicons dashicons-no-alt" data-index="'+select.index+'"></i></span>')
   ...
   renderOptions: function(){
      ...
      this.options.forEach(function(option, index){
         var childHTML = $.template(
            '<a data-index="'+index+'" data-id="'+option.value+'" class="'+option.state+'">'+ option.html +'</a>')
